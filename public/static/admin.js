...
function renderLogin(error = '') {
  root.innerHTML = `
    <div class="max-w-md mx-auto card">
      <h2 class="text-2xl font-bold mb-4"><i class="fas fa-lock text-orange-500 mr-2"></i>Admin Login</h2>
      <p class="text-gray-600 mb-4 text-sm">Enter the admin password to manage tournaments, teams, players, games, and stats.</p>
      ${error ? `<div class="bg-red-50 text-red-700 p-2 rounded mb-3 text-sm">${error}</div>` : ''}
      <input type="password" id="admin-pw" class="w-full border rounded px-3 py-2 mb-3" placeholder="Admin password" autofocus />
      <button id="admin-login-btn" class="w-full bg-orange-500 text-white py-2 rounded hover:bg-orange-600">Sign In</button>
    </div>
  `
...
function render() {
  ...
  root.innerHTML = `
    <div class="mb-4 flex flex-wrap gap-2 border-b">
      ...
      <button id="logout-btn" class="ml-auto px-3 py-2 text-sm text-gray-600 hover:text-red-600"><i class="fas fa-sign-out-alt mr-1"></i>Logout</button>
    </div>
...
function tabBtn(id, label, icon) {
  ...
  return `<button data-tab="${id}" class="px-3 py-2 text-sm ${active ? 'border-b-2 border-orange-500 text-orange-600 font-semibold' : 'text-gray-600 hover:text-orange-600'}"><i class="fas ${icon} mr-1"></i>${label}</button>`
}
...
function renderTournamentsTab(c) {
  c.innerHTML = `
    <div class="card mb-4">
      <h3 class="font-bold mb-3"><i class="fas fa-plus text-green-500 mr-2"></i>New Tournament</h3>
      <div class="grid md:grid-cols-3 gap-2">
        <input id="t-name" class="border rounded px-3 py-2" placeholder="Tournament name" />
        <input id="t-loc" class="border rounded px-3 py-2" placeholder="Location" />
        <button id="t-add" class="bg-green-500 text-white rounded px-4 py-2 hover:bg-green-600">Add</button>
      </div>
    </div>
    <div class="card">
      <h3 class="font-bold mb-3">Existing</h3>
      <table class="w-full text-sm">
        <thead class="bg-gray-100"><tr><th class="text-left p-2">ID</th><th class="text-left p-2">Name</th><th class="text-left p-2">Location</th><th class="p-2"></th></tr></thead>
        <tbody>${state.tournaments.map(t => `
          <tr class="border-b"><td class="p-2">${t.id}</td><td class="p-2">${escapeHTML(t.name)}</td><td class="p-2">${escapeHTML(t.location || '')}</td>
          <td class="p-2 text-right"><button class="text-red-600 hover:underline" data-del-t="${t.id}">Delete</button></td></tr>
        `).join('')}</tbody>
      </table>
    </div>
  `
...
function renderTeamsTab(c) {
  c.innerHTML = `
    <div class="card mb-4">
      <label class="block text-sm text-gray-600 mb-1">Tournament</label>
      <select id="team-tour" class="border rounded px-3 py-2 mb-3 w-full md:w-auto">
    ...
    <div id="team-list" class="card"><div class="text-gray-500">Pick a tournament...</div></div>
  `
...
function renderTeamFormArea() {
  ...
  area.innerHTML = `
    <div class="grid md:grid-cols-3 gap-2">
      <input id="team-name" class="border rounded px-3 py-2" placeholder="Team name" />
      <input id="team-coach" class="border rounded px-3 py-2" placeholder="Coach (optional)" />
      <button id="team-add" class="bg-green-500 text-white rounded px-4 py-2 hover:bg-green-600">Add Team</button>
    </div>
  `
...
function renderTeamList() {
  ...
  if (!state.selectedTournamentId) { list.innerHTML = '<div class="text-gray-500">Pick a tournament...</div>'; return }
  list.innerHTML = `
    <h3 class="font-bold mb-3">Teams</h3>
    ${state.teams.length === 0
      ? '<div class="text-gray-500">No teams yet.</div>'
      : `<table class="w-full text-sm">
          <thead class="bg-gray-100"><tr><th class="text-left p-2">ID</th><th class="text-left p-2">Name</th><th class="text-left p-2">Coach</th><th class="p-2"></th></tr></thead>
          <tbody>${state.teams.map(t => `
            <tr class="border-b"><td class="p-2">${t.id}</td><td class="p-2">${escapeHTML(t.name)}</td><td class="p-2">${escapeHTML(t.coach || '')}</td>
            <td class="p-2 text-right"><button class="text-red-600 hover:underline" data-del-team="${t.id}">Delete</button></td></tr>
          `).join('')}</tbody>
        </table>`
    }
  `
  ...
  c.innerHTML = `
    <div class="card mb-4">
      <div class="grid md:grid-cols-2 gap-2 mb-3">
        <select id="pl-tour" class="border rounded px-3 py-2">
          <option value="">Select tournament...</option>
          ${state.tournaments.map(t => `<option value="${t.id}" ${state.selectedTournamentId==t.id?'selected':''}>${escapeHTML(t.name)}</option>`).join('')}
        </select>
        <select id="pl-team" class="border rounded px-3 py-2">
    ...
    <div id="pl-list" class="card"><div class="text-gray-500">Pick a team...</div></div>
  `
...
function renderPlayerFormArea() {
  ...
  area.innerHTML = `
    <div class="grid md:grid-cols-5 gap-2">
      <input id="p-name" class="border rounded px-3 py-2" placeholder="Player name" />
      <input id="p-num" type="number" class="border rounded px-3 py-2" placeholder="Jersey #" />
      <input id="p-pos" class="border rounded px-3 py-2" placeholder="Pos" />
      <input id="p-ht" class="border rounded px-3 py-2" placeholder='Height (e.g. 6"1\\')' />
      <button id="p-add" class="bg-green-500 text-white rounded px-4 py-2 hover:bg-green-600">Add Player</button>
    </div>
  `
...
function renderPlayerList() {
  ...
  if (!state.selectedTeamForPlayer) { list.innerHTML = '<div class="text-gray-500">Pick a team...</div>'; return }
  list.innerHTML = `
    <h3 class="font-bold mb-3">Players</h3>
    ${state.players.length === 0
      ? '<div class="text-gray-500">No players yet.</div>'
      : `<table class="w-full text-sm">
          <thead class="bg-gray-100"><tr><th class="text-left p-2">ID</th><th class="text-left p-2">#</th><th class="text-left p-2">Name</th><th class="text-left p-2">Pos</th><th class="text-left p-2">Height</th><th class="p-2"></th></tr></thead>
          <tbody>${state.players.map(p => `
            <tr class="border-b">
              ...
              <td class="p-2 text-right"><button class="text-red-600 hover:underline" data-del-p="${p.id}">Delete</button></td>
            </tr>
          `).join('')}</tbody>
        </table>`
    }
...
function renderGamesTab(c) {
  c.innerHTML = `
    <div class="card mb-4">
      <label class="block text-sm text-gray-600 mb-1">Tournament</label>
      <select id="g-tour" class="border rounded px-3 py-2 mb-3 w-full md:w-auto">
        <option value="">Select tournament...</option>
        ${state.tournaments.map(t => `<option value="${t.id}" ${state.selectedTournamentId==t.id?'selected':''}>${escapeHTML(t.name)}</option>`).join('')}
      </select>
      <div id="g-form"></div>
    </div>
    <div id="g-list" class="card"><div class="text-gray-500">Pick a tournament...</div></div>
  `
...
function renderGameForm() {
  ...
  f.innerHTML = `
    <div class="grid md:grid-cols-6 gap-2">
      <select id="g-home" class="border rounded px-3 py-2"><option value="">Home...</option>${opts}</select>
      <input id="g-hs" type="number" class="border rounded px-3 py-2" placeholder="Home score" />
      <select id="g-away" class="border rounded px-3 py-2"><option value="">Away...</option>${opts}</select>
      <input id="g-as" type="number" class="border rounded px-3 py-2" placeholder="Away score" />
      <input id="g-date" type="date" class="border rounded px-3 py-2" />
      <button id="g-add" class="bg-green-500 text-white rounded px-4 py-2 hover:bg-green-600">Add Game</button>
    </div>
    <input id="g-venue" class="border rounded px-3 py-2 mt-2 w-full" placeholder="Venue (optional)" />
  `
...
function renderGameList() {
  ...
  list.innerHTML = `
    ...
    ${state.games.length === 0
      ? '<div class="text-gray-500">No games yet.</div>'
      : `<table class="w-full text-sm">
          <thead class="bg-gray-100"><tr><th class="text-left p-2">ID</th><th class="text-left p-2">Date</th><th class="text-left p-2">Matchup</th><th class="p-2"></th></tr></thead>
          <tbody>${state.games.map(g => `
            <tr class="border-b">
              ...
              <td class="p-2 text-right"><button class="text-red-600 hover:underline" data-del-g="${g.id}">Delete</button></td>
            </tr>
          `).join('')}</tbody>
        </table>`
    }
...
function renderStatsTab(c) {
  c.innerHTML = `
    <div class="card mb-4">
      <h3 class="font-bold mb-3"><i class="fas fa-chart-bar text-orange-500 mr-2"></i>Enter Player Stats</h3>
      <p class="text-sm text-gray-600 mb-3">Enter the player's totals (PTS / REB / AST) for a single game. Submitting again for the same game and player will overwrite existing stats.</p>
      <div class="grid md:grid-cols-2 gap-2 mb-3">
        <select id="s-tour" class="border rounded px-3 py-2">
          <option value="">Select tournament...</option>
          ${state.tournaments.map(t => `<option value="${t.id}" ${state.selectedTournamentId==t.id?'selected':''}>${escapeHTML(t.name)}</option>`).join('')}
        </select>
        <select id="s-game" class="border rounded px-3 py-2"><option value="">Select game...</option></select>
      </div>
      <div class="grid md:grid-cols-2 gap-2 mb-3">
        <select id="s-team" class="border rounded px-3 py-2"><option value="">Select team...</option></select>
        <select id="s-player" class="border rounded px-3 py-2"><option value="">Select player...</option></select>
      </div>
      <div class="grid grid-cols-3 gap-2 mb-3">
        <input id="s-pts" type="number" min="0" class="border rounded px-3 py-2" placeholder="PTS" />
        <input id="s-reb" type="number" min="0" class="border rounded px-3 py-2" placeholder="REB" />
        <input id="s-ast" type="number" min="0" class="border rounded px-3 py-2" placeholder="AST" />
      </div>
      <button id="s-save" class="bg-orange-500 text-white px-4 py-2 rounded hover:bg-orange-600">Save Stats</button>
...
function renderSettingsTab(c) {
  c.innerHTML = `
    <div class="card max-w-md">
      <h3 class="font-bold mb-3"><i class="fas fa-key text-orange-500 mr-2"></i>Change Admin Password</h3>
      <input id="new-pw" type="password" class="border rounded px-3 py-2 w-full mb-2" placeholder="New password (min 4 chars)" />
      <input id="new-pw2" type="password" class="border rounded px-3 py-2 w-full mb-2" placeholder="Confirm new password" />
      <button id="pw-save" class="bg-orange-500 text-white px-4 py-2 rounded hover:bg-orange-600">Update Password</button>
